...
if not os.path.isdir(f"prediction_result/{path}/T{THRESHOLD}"):
    os.system(f"mkdir -p prediction_result/{path}/T{THRESHOLD}")

# this is for one model selecting positive tiles only
res_file = './mmdetection/work_dirs/tile_libra_faster_rcnn_x101_64x4d_fpn_1x_new/20191115_234658_libra_x101_epoch_15_only_pos.bbox.json'
pred = json.load(open(res_file, "r"))
...
for img in tqdm(res_df_filtered['img'].unique()):
    ...
    json.dump(preds_final_filtered,
              open(f"submissions/{path}/T{THRESHOLD}/{img.replace('kfb', 'json')}", "w"))

no_pos = set(all_test_sizes['filename'].unique()) - set(res_df_filtered['img'].unique())
for image in no_pos:
    json.dump([], open(f'./submissions/{path}/T{THRESHOLD}/{image.replace(".kfb", "")}.json', 'w'))
